"""
SQL Injection Testing Mutation Engine
Generates unique variants of sql injection testing vulnerabilities
"""
...
class TestingInjectionSqliMutation(MutationEngine):
    """
    Mutation engine for SQL Injection Testing vulnerabilities
    """

    def mutate(self, blueprint: VulnerabilityBlueprint, difficulty: int) -> MachineConfig:
        """Generate unique sql injection testing machine configuration"""
        ...
        if variant == "Basic SQL Injection":
            config = self._generate_basic_sql_injection(blueprint, difficulty)
        else:
            config = self._generate_basic_sql_injection(blueprint, difficulty)
    ...
    def _generate_basic_sql_injection(self, blueprint: VulnerabilityBlueprint, difficulty: int) -> Dict:
        """Generate Basic SQL Injection vulnerability configuration"""

        context = self.select_random(blueprint.mutation_axes.get('contexts', ['login_form']))
        sink = self.select_random(blueprint.mutation_axes.get('sinks', ['mysql_query']))
        ...
        return {
            ...
            'metadata': {
                'exploit_hints': hints,
                'vulnerability_type': 'Basic SQL Injection',
