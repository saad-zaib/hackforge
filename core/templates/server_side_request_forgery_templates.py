"""
Server-Side Request Forgery Vulnerability Templates
...
class ServerSideRequestForgeryTemplate(BaseTemplate):
    """
    Template generator for server-side request forgery vulnerabilities
    """
    ...
    def generate_code(self) -> str:
        ...
        if variant == "Basic SSRF":
            return self._generate_basic_ssrf()
        elif variant == "Blind SSRF":
            return self._generate_blind_ssrf()
        elif variant == "SSRF with Protocol Smuggling":
            return self._generate_ssrf_with_protocol_smuggling()
        else:
            return self._generate_basic_ssrf()

    def _generate_basic_ssrf(self) -> str:
        """Generate Basic SSRF vulnerable application with themed UI"""
...
<html>
<head>
    <title>Basic SSRF Challenge</title>
    {fonts_import}
    <style>
{theme_css}
    </style>
...
<body>
    ...
    def _generate_blind_ssrf(self) -> str:
        """Generate Blind SSRF vulnerable application with themed UI"""
...
<html>
<head>
    <title>Blind SSRF Challenge</title>
    {fonts_import}
    <style>
{theme_css}
    </style>
...
<body>
    ...
    def _generate_ssrf_with_protocol_smuggling(self) -> str:
        """Generate SSRF with Protocol Smuggling vulnerable application with themed UI"""
...
/**
 * Hackforge Machine: {self.machine_id}
 * Vulnerability: SSRF with Protocol Smuggling
 * Theme: {self.theme['name']}
 * Difficulty: {self.difficulty}/5
 */
?>
...
<html>
<head>
    <title>SSRF with Protocol Smuggling Challenge</title>
    {fonts_import}
    <style>
{theme_css}
    </style>
...
<body>
    <div class="container">
        <h1>SSRF with Protocol Smuggling</h1>
        <p>Context: {context}</p>
    ...
    def generate_dockerfile(self) -> str:
        """Generate Dockerfile for server-side request forgery vulnerabilities"""
