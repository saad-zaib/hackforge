"""
Cross-Site Request Forgery Vulnerability Templates
...
class BrokenAccessControlTemplate(BaseTemplate):
    """
    Template generator for cross-site request forgery vulnerabilities
    """
    ...
    def generate_code(self) -> str:
        ...
        if variant == "Basic CSRF":
            return self._generate_basic_csrf()
        elif variant == "CSRF with SameSite Bypass":
            return self._generate_csrf_with_samesite_bypass()
        elif variant == "JSON CSRF":
            return self._generate_json_csrf()
        else:
            return self._generate_basic_csrf()

    def _generate_basic_csrf(self) -> str:
        """Generate Basic CSRF vulnerable application with themed UI"""
...
<html>
<head>
    <title>Basic CSRF Challenge</title>
    {fonts_import}
    <style>
{theme_css}
    </style>
...
<body>
    ...
    def _generate_csrf_with_samesite_bypass(self) -> str:
        """Generate CSRF with SameSite Bypass vulnerable application with themed UI"""
...
/**
 * Hackforge Machine: {self.machine_id}
 * Vulnerability: CSRF with SameSite Bypass
 * Theme: {self.theme['name']}
 * Difficulty: {self.difficulty}/5
 */
?>
...
<html>
<head>
    <title>CSRF with SameSite Bypass Challenge</title>
    {fonts_import}
    <style>
{theme_css}
    </style>
...
<body>
    <div class="container">
        <h1>CSRF with SameSite Bypass</h1>
        <p>Context: {context}</p>
    ...
    def _generate_json_csrf(self) -> str:
        """Generate JSON CSRF vulnerable application with themed UI"""
...
<html>
<head>
    <title>JSON CSRF Challenge</title>
    {fonts_import}
    <style>
{theme_css}
    </style>
...
<body>
    ...
    def generate_dockerfile(self) -> str:
        """Generate Dockerfile for cross-site request forgery vulnerabilities"""
